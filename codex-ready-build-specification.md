...
Customer can log in using email/password or phone/password.

Customer can create an order manually by selecting items from item master list.

Customer can create an order via scan upload; system extracts lines; customer must confirm/edit before submission.

Customer must choose pickup store for pickup, and must enter address/contact for delivery.
...
Staff queue shows correct orders by role and sorted by queue number.

Staff can edit confirmed details (notes, delivery fee info, substitutions) with audit log.

Staff can regenerate XLS; each regeneration creates a new version.

Staff can download the latest confirmed XLS.

XLS

Generated XLS always matches:
...
Email routing is correct for Cabramatta pickup, Lidcombe pickup, and delivery orders.

Email contains summary + staff portal link + XLS attachment or download link.
